...
type Elected struct {
	...
	// LeadershipData is the replica data associated with each leader.
...
func (e *Elector) Elect(ctx context.Context) (context.Context, *Elected, error) {
	...
	var ldata []*anypb.Any
	for {
		ldata, err = e.quorumReconcile(ctx, resp)
		if err != nil {
	...
	go func() {
		...
		for {
			...
			if err := e.haveQuorum(ctx, uint64(len(ldata)), r); err != nil {
...
func (e *Elector) haveQuorum(ctx context.Context, expTotal uint64, resp *clientv3.GetResponse) error {
	var selfFound bool
	...
	for _, kv := range resp.Kvs {
		...
		if currentPartitionTotal != lead.Total {
			return fmt.Errorf("%s: partition total changed: %d -> %d, fatal leadership error", string(kv.Key), lead.Total, currentPartitionTotal)
		}
		...
		if string(kv.Key) == e.leaderKey {
			...
			if lead.Uid != e.uid {
				return fmt.Errorf("own uid changed: %d -> %d, fatal leadership error", e.uid, lead.Uid)
			}
			if !proto.Equal(lead.ReplicaData, e.replicaData) {
				return fmt.Errorf("own replica data changed: %v -> %v, fatal leadership error", e.replicaData, lead.ReplicaData)
			}
		}
	}
...
// partition number.
func (e *Elector) quorumReconcile(ctx context.Context, resp *clientv3.GetResponse) ([]*anypb.Any, error) {
	selfFound := false
	allAgree := true

	currentPartitionTotal := uint64(resp.Count)

	ldata := make([]*anypb.Any, resp.Count)
	...
	for i, kv := range resp.Kvs {
		...
		// make any progress!
		if existing.Total != currentPartitionTotal {
			allAgree = false
		}
	}
	...
	if !selfFound {
		return nil, fmt.Errorf("own leadership key has been deleted, fatal leadership error")
	}

	// Not all agree, signal to wait for the next informer event.
	if !allAgree {
		return nil, nil
	}
...
// TODO: @joshanl
// reconcileSelf ensures that out leadership value has not been changed by
// someone else, and updating the stored data if the partition total has
// changed.
func (e *Elector) reconcileSelf(ctx context.Context, self *mvccpb.KeyValue, selfLeader *stored.Leadership, partitionTotal uint64) error {
	if selfLeader.Uid != e.uid {
		return fmt.Errorf("own uid changed: %d -> %d, fatal leadership error", e.uid, selfLeader.Uid)
	}

	if !proto.Equal(selfLeader.ReplicaData, e.replicaData) {
		return fmt.Errorf("own replica data changed: %v -> %v, fatal leadership error", e.replicaData, selfLeader.ReplicaData)
	}

	// If the partition total has not changed, we don't need to re-elect.
	if partitionTotal == selfLeader.Total {
		return nil
	}

	// The partition total has changed to what we have previously observed, so we
	// need to update out leadership data.
